<template>
  <li
    v-for="(folder, i) in filesystem"
    :key="folder.id"
    :class="{ 'has-child': true, open: i == 0 }"
  >
    <a @click="switchDirectory(folder.id)" href="#" class="">
      <img class="svg nav-icon" src="/assets/img/svg/folder.svg" alt="folder" />
      <span class="menu-text">{{ folder.name }}</span>
      <span class="toggle-icon"></span>
    </a>
    <ul>
      <li
        :class="{ 'has-child': folder.folders.length > 0 }"
